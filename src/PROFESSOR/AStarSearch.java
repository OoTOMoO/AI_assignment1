...
public class AStarSearch extends AbstractSearchAlgorithm {
	...
	/**
	 * A basic implementation of a Priority Queue-based A* search. The core is a
	 * loop that searches the foremost entry in the queue until either a goal is
	 * found, or the queue is empty.
	 */
	public void search() {
	...
	public boolean processCurrentEntry() {
		...
		for (State s2 : successors) {
			if (!predMap.containsKey(s2)) {
				queue.add(new QueueEntry<Object>(s2, currentState, 
