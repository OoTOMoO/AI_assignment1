...
/**
 * An implementation of a BFS that does not revisit states, using a LinkedList
 * as a queue for holding the states to visit.
 * 
 * @author lackofcheese
 */
public class BreadthFirstSearch extends AbstractSearchAlgorithm {
	...
	/**
	 * A representation of the search tree; remembers which states were
	 * expanded, and the predecessor for each.
	 */
	...
	/**
	 * A basic implementation of a queue-based BFS. The core is a loop that
	 * searches the foremost entry in the queue until either a goal is found, or
	 * the queue is empty.
	 */
	public void search() {
	...
	/**
	 * Processes the current entry from the search queue, and returns true if it
	 * was the goal, and false otherwise.
	 * 
	...
	public boolean processCurrentEntry() {
		...
		for (State s2 : successors) {
			if (!predMap.containsKey(s2)) {
				queue.add(new QueueEntry<Object>(s2, currentState, currentEntry
						.getDepth() + 1, currentEntry.getTotalCost()
						+ currentState.getCost(s2),
						getHeuristic().estimate(s2), null));
			}
		}
		return false;
	}
